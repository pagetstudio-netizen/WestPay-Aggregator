...
export async function registerRoutes(
  ...
  // ─── API v1 : Statut d'une transaction par ID ─────────────────

  app.get("/api/v1/merchant/transactions/:transaction_id", sdkAuthMiddleware, async (req, res) => {
    try {
      const merchant = (req as any).sdkMerchant;
      const { transaction_id } = req.params;

      // 1. Chercher dans les paiements en attente (pending)
      const pending = await storage.getPendingPaymentByOmnipayReference(transaction_id);
      if (pending && pending.merchantId === merchant.id) {
        return res.json({
          status: "success",
          data: {
            ...
            amount: pending.amount,
            currency: "XOF",
            transaction_status: pending.status,
            payment_method: pending.paymentMethod || "mobile_money",
            country: pending.country,
            phone_number: pending.payerPhone,
            payer_name: (pending as any).payerName || null,
            created_at: pending.createdAt,
            processed_by: "RobotPay",
          },
        });
      }

      // 2. Chercher dans les transactions confirmées/échouées
      const tx = await storage.getTransactionByTxId(transaction_id);
      if (tx && tx.merchantId === merchant.id) {
        return res.json({
          status: "success",
          data: {
            ...
            payment_method: tx.operator || "mobile_money",
            country: tx.country,
            phone_number: tx.payerNumber,
            payer_name: (tx as any).payerName || null,
            created_at: tx.createdAt,
            processed_by: "RobotPay",
          },
        });
      ...
      // 3. Chercher dans les retraits (payout)
      const withdrawal = await storage.getWithdrawalByOmnipayRef(transaction_id);
      if (withdrawal && withdrawal.merchantId === merchant.id) {
        return res.json({
          status: "success",
          data: {
            transaction_id,
            internal_id: withdrawal.id,
            type: "payout",
            amount: withdrawal.amount,
            currency: "XOF",
            fee: withdrawal.fees,
            transaction_status: withdrawal.status,
            payment_method: withdrawal.operator || "mobile_money",
            country: withdrawal.country,
            phone_number: withdrawal.phone,
            created_at: withdrawal.createdAt,
            processed_by: "RobotPay",
      ...
      return res.status(404).json({
        status: "error",
        message: "Transaction introuvable ou non autorisée pour ce compte.",
      });
    } catch (err: any) {
