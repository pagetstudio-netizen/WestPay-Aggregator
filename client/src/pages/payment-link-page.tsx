    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (d.status === "confirmed") {
          ...
          clearInterval(pollingRef.current!); setOmniPolling(false);
          setFailed(true); setFailReason("Le paiement n'a pas pu être traité. Vérifiez votre solde ou votre code secret.");
        }
      } catch {}
    }, 5000);
  };
  ...
  if (error || !data) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", padding: 16 }}>
      ...
      <div style={{ textAlign: "center", maxWidth: 320 }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 14 }}>{(error as Error)?.message || "Ce lien de paiement est introuvable ou a expiré."}</p>
      </div>
    </div>
  );
          ...
          <div style={{ padding: "18px 20px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {step === 1 && (<>
              ...
              {data.link.amountType === "flexible" && (
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Montant ({currency})</p>
                  <input type="number" value={customAmount} onChange={e => setCustomAmount(e.target.value)}
                    placeholder="Ex: 5000" className="inp" data-testid="input-custom-amount" />
              ...
              {/* Operator circles */}
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, color: "#6b7280", marginBottom: 10 }}>Choisissez une méthode de paiement</p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }} data-testid="step2">
                ...
                {failed ? (
                  ...
                  <div style={{ background: "#dbeafe", borderRadius: 12, padding: 12, textAlign: "center", fontSize: 14, fontWeight: 500, color: "#1e40af" }}>
                    Cliquez ci-dessous pour valider votre paiement de {fmt(effectiveAmount)} {currency}
                  </div>
                  ...
                  <div style={{ textAlign: "center", padding: "14px 0" }}>
                    ...
                    <p style={{ fontSize: 13, color: "#6b7280", marginTop: 6 }}>
                      Demande de <strong style={{ color: "#1d4ed8" }}>{fmt(effectiveAmount)} {currency}</strong> envoyée sur votre appareil.
                    </p>
                  </div>
                  {maliOrange && (
                    <div style={{ background: "#fff7ed", border: "1.5px solid #fed7aa", borderRadius: 12, padding: "10px 14px" }}>
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "12px 20px 0" }}>
              ...
              {helpSent ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: 8 }}>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 7 }}>Nom <span style={{ color: "#ef4444" }}>*</span></label>
                    <input type="text" value={helpName} onChange={e => setHelpName(e.target.value)} placeholder="Votre nom" className="inp" style={{ fontSize: 15 }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 7 }}>Numéro WhatsApp <span style={{ fontWeight: 400, color: "#9ca3af" }}>(optionnel)</span></label>
                  ...
                  <div>
                    ...
                    <textarea value={helpMessage} onChange={e => setHelpMessage(e.target.value)} placeholder="Décrivez votre problème..." rows={4} className="inp" style={{ fontSize: 15, resize: "none", fontFamily: "inherit" }} />
                  </div>
                </div>
              )}
            </div>
              ...
              {helpSent ? (
                ...
                <button type="button"
                  ...
                  onClick={async () => {
                    ...
                    } catch { toast({ title: "Erreur réseau", description: "Vérifiez votre connexion.", variant: "destructive" }); }
