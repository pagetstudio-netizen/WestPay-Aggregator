...
export default function PaymentPage() {
  ...
  const [supportContacts, setSupportContacts] = useState<{telegram1?:string;telegram2?:string;whatsapp1?:string;whatsapp2?:string;hours?:string;hours2?:string} | null>(null);

  /* fetch support contacts once */
  useEffect(() => {
    fetch("/api/public/support-contacts").then(r => r.ok ? r.json() : null).then(d => { if (d) setSupportContacts(d); }).catch(() => {});
  }, []);
  ...
  return (
    <>
      ...
      {/* ── Help modal ── */}
      {showHelpModal && (
        <div className="overlay" onClick={e => { if (e.target===e.currentTarget) setShowHelpModal(false); }}>
          <div className="modal" style={{ textAlign:"center" }}>
            {/* Header */}
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
              <div style={{ flex:1 }} />
              <img src="/help-icon.png" alt="Aide" style={{ width:72, height:72, objectFit:"cover", borderRadius:"50%", border:"3px solid #e8f5e9", boxShadow:"0 4px 16px rgba(0,176,80,.18)" }} />
            ...
            <p style={{ fontWeight:800, fontSize:18, color:"#111", marginBottom:4 }}>Besoin d'aide ?</p>
            <p style={{ fontSize:13, color:"#6b7280", marginBottom:20, lineHeight:1.5 }}>
              Notre équipe support est disponible pour vous aider avec votre paiement.
            </p>

            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {supportContacts?.whatsapp1 && (
                <a href={`https://wa.me/${supportContacts.whatsapp1.replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer"
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 16px", borderRadius:14, background:"#e8f5e9", border:"1.5px solid #c8e6c9", textDecoration:"none", transition:"opacity .15s" }}>
                  ...
                  <div style={{ textAlign:"left" }}>
                    <p style={{ fontSize:12, fontWeight:700, color:"#1b5e20", margin:0 }}>WhatsApp</p>
                    <p style={{ fontSize:14, fontWeight:600, color:"#2e7d32", margin:0 }}>{supportContacts.whatsapp1}</p>
                    {supportContacts.whatsapp2 && <p style={{ fontSize:12, color:"#388e3c", margin:"2px 0 0" }}>{supportContacts.whatsapp2}</p>}
                  </div>
                </a>
              )}

              {supportContacts?.telegram1 && (
                <a href={`https://t.me/${supportContacts.telegram1.replace("@","")}`} target="_blank" rel="noopener noreferrer"
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 16px", borderRadius:14, background:"#e3f2fd", border:"1.5px solid #bbdefb", textDecoration:"none" }}>
                  <div style={{ width:40, height:40, borderRadius:12, background:"#229ed9", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  ...
                  <div style={{ textAlign:"left" }}>
                    <p style={{ fontSize:12, fontWeight:700, color:"#0d47a1", margin:0 }}>Telegram</p>
                    <p style={{ fontSize:14, fontWeight:600, color:"#1565c0", margin:0 }}>{supportContacts.telegram1}</p>
                    {supportContacts.hours && <p style={{ fontSize:11, color:"#1976d2", margin:"2px 0 0" }}>{supportContacts.hours}</p>}
                    {supportContacts.telegram2 && <p style={{ fontSize:12, color:"#1976d2", margin:"2px 0 0" }}>{supportContacts.telegram2}{supportContacts.hours2 ? ` · ${supportContacts.hours2}` : ""}</p>}
                  </div>
                </a>
              )}

              {/* Fallback if no contacts configured */}
              {!supportContacts?.whatsapp1 && !supportContacts?.telegram1 && (
                <div style={{ padding:"14px", background:"#f9fafb", borderRadius:14, border:"1.5px solid #e5e7eb" }}>
                  <p style={{ fontSize:13, color:"#6b7280", margin:0 }}>Contactez le marchand directement pour toute assistance.</p>
                </div>
              )}
            </div>

            <button onClick={() => setShowHelpModal(false)}
              style={{ marginTop:18, width:"100%", padding:"12px 16px", borderRadius:12, border:"1.5px solid #e5e7eb", background:"#f9fafb", cursor:"pointer", fontSize:14, fontWeight:600, color:"#374151" }}>
              Fermer
            </button>
          </div>
        </div>
